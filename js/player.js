...
const player = {
  ...
  set_position: function (mapName) {
    var map = config.maps[mapName]
  ...
  set_offsets: function (offsetX, offsetY) {
  ...
  move: function (direction) {
    ...
    var y = player.offset_y + (player.y / config.tile_height)
    var prevSteps = this.steps
  ...
  will_collide: function (x, y) {
    var nextTile = map.map_ptr.layout[x + (y * map.map_ptr.width)] - 1
    if (this.collide_tiles.indexOf(nextTile) > -1 || map.get_npc(x, y) !== null) {
      return true
    }
    return false
  },
  ...
  door: function () {
    const x = player.offset_x + (player.x / config.tile_width)
    const y = player.offset_y + (player.y / config.tile_height)
    let door = null
